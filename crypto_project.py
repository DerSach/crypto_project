...
def print_portfolios(df):
    ...
    if len(df) == 0:
        ...
        print(df)
    if os.path.isfile('crypto_historical_profit.csv'):
        hist_profit_df = pd.read_csv('crypto_historical_profit.csv', index_col=[0])
        print()
...
def get_historical_profit_df(profit, currency):
    ...
    hist_profit_df.to_csv('crypto_historical_profit.csv')
